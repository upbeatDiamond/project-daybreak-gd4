...
Therefore, this hacky workaround has been made by using the face that custom_minimum_size
...
@export var alignment = "CENTER_BOTTOM" # (String, "CENTER", "CENTER_BOTTOM")
...
func _ready():
	default_img_size = anim_img.sprite_frames.get_frame_texture("default", 0).get_size() 
	set_content_margin()
	align_icon()
	
# Set spacing to be minimum animation frame size and content margin.
func set_content_margin():
	self.custom_minimum_size = default_img_size + content_margin
	align_icon()
...
func align_icon():
	...
	if alignment == "CENTER_BOTTOM":
		...
		var anim_img_size = anim_img.sprite_frames.get_frame_texture(anim_img.animation, 0).get_size()
		anim_img.position = (self.custom_minimum_size) / 2 + Vector2(0, content_margin.y / 2) + \
				Vector2(0, (default_img_size.y - anim_img_size.y) / 2)
				#				(default_img_size - anim_img_size) / 2
	else:
		# Center
		anim_img.position = self.custom_minimum_size / 2
