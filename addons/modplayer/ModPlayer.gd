...
# 1秒間処理する回数
@export_range (60, 480) var sequence_per_seconds:int = 120
## スレッドでの動作にするか
@export var no_thread_mode:bool = false
...
func _process_tick_for_channel( channel:GodotModPlayerChannelStatus, note:Mod.ModPatternNote, disable_channel_row:bool ) -> void:
	...
	match note.effect_command:
		...
		0x01:	# Portament up
			if not disable_channel_row:
				channel.pitch.value = maxi( channel.pitch.value - note.effect_param, 0 )
		0x02:	# Portament down
			if not disable_channel_row:
		...
		0x05:	# Portament + Volume slide
			channel.pitch.update( )
			channel.volume = clampi( channel.volume + ( note.effect_param >> 4 ) - ( note.effect_param & 0x0F ), 0, 64 )
		...
		0x19:	# Pannning slide
			if not disable_channel_row:
				channel.panning = clampi( channel.panning + ( note.effect_param >> 4 ) - ( note.effect_param & 0x0F ), 0, 255 )
		0x1B:	# Multi retrig note
			if ( ( self.processed_tick_count - 1 ) % ( ( note.effect_param & 0xF0 ) >> 4 ) ) == 0:
				channel.retrig( )
			if disable_channel_row:
				match note.effect_param & 0x0F:
					0:
						pass
					1:
