...
func _ready() -> void:
	...
	GlobalFieldEvents.cell_selected.connect(_on_cell_selected)
	...
	if not _focus.is_moving():
		...
		if move_dir:
			...
			if not is_cell_blocked(target_cell) and \
					not GlobalFieldEvents.did_gp_move_to_cell_this_frame(target_cell):
...
func _on_focus_arriving(excess_distance: float) -> void:
	...
	if not _waypoints.is_empty():
		while not _waypoints.is_empty() and excess_distance > 0:
			if is_cell_blocked(_waypoints[0]) or GlobalFieldEvents.did_gp_move_to_cell_this_frame(_waypoints[0]):
				return
	...
	# There is no path to follow, so defer to movement keys or buttons that are currently held down.
		...
		if pathfinder.has_cell(next_cell) and not is_cell_blocked(next_cell) and \
				not GlobalFieldEvents.did_gp_move_to_cell_this_frame(next_cell):
...
func _on_cell_selected(cell: Vector2i) -> void:
	if not _focus.is_moving():
		...
		if _waypoints.size() > 1:
			GlobalFieldEvents.player_path_set.emit(_focus, _waypoints.back())
